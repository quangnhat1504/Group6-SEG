...
def main() -> None:
    ...
    save_run(config.outputs.run_dir / f"{args.split}_dense_{slug}.csv", dense_run)
    save_run(config.outputs.run_dir / f"{args.split}_hybrid_{slug}.csv", hybrid_run)
    ...
    metrics_path = config.outputs.run_dir / f"{args.split}_dense_ablation_{slug}_metrics.json"
