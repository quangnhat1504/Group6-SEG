"""Master script: run full SEG pipeline on SciDocs."""
...
RUN_DIR = Path("runs/scidocs"); RUN_DIR.mkdir(parents=True, exist_ok=True)
...
def q_idf(text, idf):
    ...
    return sum(idf.get(s,0.0) for s in stems)/len(stems) if stems else 0.0

def arf(bm25, dense, queries, idf, scale=1.0, max_w=0.9, k=60, top_k=100):
    idfs=[q_idf(q.text, idf) for q in queries]; center=np.median(idfs)
    fused={}
    for q in queries:
        midf=q_idf(q.text, idf)
        w_b=0.05+(1.0/(1.0+math.exp(-scale*(midf-center))))*(max_w-0.05)
        scores=defaultdict(float)
        for r,(did,_) in enumerate(bm25.get(q.query_id,[]),1): scores[did]+=w_b/(k+r)
        for r,(did,_) in enumerate(dense.get(q.query_id,[]),1): scores[did]+=1.0/(k+r)
...
def main():
    ...
    print(f"SciDocs: {len(docs)} docs, {len(queries)} queries, {len(qrels)} judged", flush=True)
    ...
    print(f"  nDCG@10={bm['ndcg@10']:.4f} MAP@10={bm['map@10']:.4f} "
          f"R@10={bm['recall@10']:.4f} R@100={bm['recall@100']:.4f} "
          f"({time.perf_counter()-t0:.0f}s)", flush=True)

    # 2. BGE-base (GPU)
    print("\n[2/6] BGE-base...", flush=True)
    t0=time.perf_counter()
    ...
    print(f"  nDCG@10={bg['ndcg@10']:.4f} MAP@10={bg['map@10']:.4f} "
          f"R@10={bg['recall@10']:.4f} R@100={bg['recall@100']:.4f} "
          f"({time.perf_counter()-t0:.0f}s)", flush=True)

    # 3. SciNCL (GPU, fast since cached)
    print("\n[3/6] SciNCL...", flush=True)
    t0=time.perf_counter()
    ...
    print(f"  nDCG@10={sc['ndcg@10']:.4f} MAP@10={sc['map@10']:.4f} "
          f"R@10={sc['recall@10']:.4f} R@100={sc['recall@100']:.4f} "
          f"({time.perf_counter()-t0:.0f}s)", flush=True)

    # 4. Hybrid RRF
    print("\n[4/6] Hybrid RRF...", flush=True)
    ...
    print(f"  BGE RRF: nDCG@10={rr['ndcg@10']:.4f}  SciNCL RRF: nDCG@10={rsc['ndcg@10']:.4f}", flush=True)

    # 5. Adaptive RRF
    print("\n[5/6] Adaptive RRF...", flush=True)
    idf=build_idf(docs)
    print(f"  IDF vocab: {len(idf)} terms", flush=True)
    ...
    print(f"  nDCG@10={af['ndcg@10']:.4f} MAP@10={af['map@10']:.4f}", flush=True)

    # 6. CE Rerank (GPU)
    print("\n[6/6] CE Rerank...", flush=True)
    ...
    print(f"  CE on ARF: nDCG@10={cea['ndcg@10']:.4f}  "
          f"CE on SciNCL: nDCG@10={ces['ndcg@10']:.4f}  "
          f"({el/len(queries)*1000:.0f}ms/q)", flush=True)
    ...
    for name,m in [("BM25",bm),("SciNCL",sc),("BGE-base",bg),
                   ("Hybrid RRF",rr),("Adaptive RRF",af),
                   ("CE on ARF",cea),("CE on SciNCL RRF",ces)]:
        print(f"{name:<20} {m['ndcg@10']:>8.4f} {m['map@10']:>8.4f} "
              f"{m['recall@10']:>8.4f} {m['recall@100']:>8.4f}")

    print(f"\nBGE-base vs SciNCL: +{bg['ndcg@10']-sc['ndcg@10']:.4f}")
    print(f"Adaptive RRF vs BGE-base: {af['ndcg@10']-bg['ndcg@10']:+.4f}")
    print(f"CE on ARF vs ARF: {cea['ndcg@10']-af['ndcg@10']:+.4f}")
