"""Generate positioning note documenting SEG's relationship to Al-Joofi et al. (MDPI 2025).

Produces a structured Markdown document with three sections:
  1. Shared findings between SEG and Al-Joofi et al.
...
def generate_positioning_note() -> str:
    """Return the full Markdown content for the positioning note."""
    return """\
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate positioning note for SEG vs Al-Joofi et al. (MDPI 2025)"
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=OUTPUT_DIR / "positioning_note_al_joofi.md",
        help="Output path for the positioning note (default: reports/tables/positioning_note_al_joofi.md)",
    )
    args = parser.parse_args()

    content = generate_positioning_note()

    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(content, encoding="utf-8")
    print(f"Positioning note written to: {args.output}")
